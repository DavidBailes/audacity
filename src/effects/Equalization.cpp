...
void JSlider::AdjustSlider(const wxKeyEvent& event)
{
   switch( event.GetKeyCode() )
   {
   case WXK_DOWN:
   ...
   case WXK_LEFT:
      ...
      break;
   case WXK_PAGEDOWN:
      if (slider->GetValue() - slider->GetPageSize() < slider->GetMin())
         slider->SetValue(slider->GetMin());
      else
         slider->SetValue(slider->GetValue() - slider->GetPageSize());
      break;
   case WXK_PAGEUP:
      if (slider->GetValue() + slider->GetPageSize() > slider->GetMax())
   ...
   wxCommandEvent e( wxEVT_COMMAND_SLIDER_UPDATED, GetId() );
...
// This handles slider change events generated by using the mouse
void JSlider::OnSlider(wxCommandEvent& event)
{
   event.SetId(GetId());
